function convertTableToStackedList() {
  ...
  rows.forEach(row => {
    ...
    // Iterate over the remaining cells (skip the first) and create content for each list item
    Array.from(row.querySelectorAll("td")).slice(1).forEach((cell, index) => {
      var header = headers[index + 1]; // Use index + 1 to get the right header
      var content = cell.innerText;

      if (cell.querySelector('a')) {  // If the cell has a link
        var link = cell.querySelector('a').getAttribute('href');
        content = '<a href="' + link + '" target="_blank">' + content + '</a>';
      }

      // Exclude the column title "Image preview"
...
document.addEventListener("DOMContentLoaded", function () {
  handleResize(); // Perform initial conversion based on screen size
});
